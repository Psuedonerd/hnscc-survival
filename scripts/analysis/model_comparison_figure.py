...
def prepare_results(df, top_n):
    ...
    if {"Condition", "Model"}.issubset(df.columns):
        ...
        prepared = df.sort_values("Test_C", ascending=False).head(top_n).copy()
        prepared["Label"] = prepared["Experiment"].astype(str) + "\n" + prepared["Config"].astype(str)
    else:
        prepared = df.head(top_n).copy()
...
def main():
    ...
    ax.set_ylim(0.45, max(0.75, np.nanmax(plot_df[["Train_C", "Test_C"]].to_numpy()) + 0.05))
